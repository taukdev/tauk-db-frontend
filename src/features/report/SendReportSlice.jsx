...
const sendReportSlice = createSlice({
  ...
  reducers: {
    ...
    toggleSelectAllOnPage(state, action) {
      ...
      const allSelected = ids.every((id) => state.selected.includes(id));
      if (allSelected) {
        state.selected = state.selected.filter((id) => !ids.includes(id));
      } else {
        state.selected = Array.from(new Set([...state.selected, ...ids]));
      }
    },
  },
...
export const { setSearch, setPage, setPerPage, toggleSelect, clearSelected, toggleSelectAllOnPage } = sendReportSlice.actions;
